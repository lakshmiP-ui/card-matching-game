...
class CardMatchingGame {
    ...
    const newMatchedIndices = new Set();

    this.gameState.cards.forEach((card, index) => {
        if (card.isMatched) {
            newMatchedIndices.add(index);
        }
    });
    ...
    this.gameState.cards.forEach((card, index) => {
        ...
        // Add just-matched class if card just got matched this render
        if (card.isMatched && !this.previousMatchedIndices.has(index)) {
            cardElement.classList.add('just-matched');
        }

        gameBoard.appendChild(cardElement);
    });

    // Update previousMatchedIndices for next render
    ...
    if (card1.symbol !== card2.symbol) {
        const cardEl1 = document.querySelector(
            `[data-row="${card1.position.row}"][data-col="${card1.position.col}"]`
        );
        const cardEl2 = document.querySelector(
            `[data-row="${card2.position.row}"][data-col="${card2.position.col}"]`
        );

        // Add red mismatch animation
        cardEl1.classList.add('mismatch','flipped');
        cardEl2.classList.add('mismatch','flipped');

        setTimeout(async () => {
            cardEl1.classList.remove('mismatch','flipped');
            cardEl2.classList.remove('mismatch','flipped');

            // Ask backend to flip back
            await fetch(`/api/game/${this.gameId}/flipback`, {
                method: 'POST',
            });
